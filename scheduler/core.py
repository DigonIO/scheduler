...
# NOTE new API design
# NOTE minutly gibt es nicht, nur every_minute und minute_by_minute => Problem
...
TimingTypeDay = Union[Weekday, dt.time, tuple[Weekday, dt.time]]
TimingTypeWeekly = Union[TimingTypeDay, list[TimingTypeDay]]
...
class Scheduler:  # in core
    ...
    def cyclic(
        ...
        job = self.__schedule(job_type=JobType.CYCLIC, timing=timing, handle=handle)
    ...
    def minute(
        ...
        job = self.__schedule(job_type=JobType.MINUTE, timing=timing, handle=handle)
    ...
    def hourly(
        ...
        job = self.__schedule(job_type=JobType.HOURLY, timing=timing, handle=handle)
    ...
    def daily(
        ...
        job = self.__schedule(job_type=JobType.DAILY, timing=timing, handle=handle)
    ...
    def weekly(
        ...
        job = self.__schedule(job_type=JobType.WEEKLY, timing=timing, handle=handle)
    ...
    def once(
        ...
        job = self.__schedule(job_type=JobType.ONCE, timing=timing, handle=handle)


class AbstractJob:  # in util
    pass
...
class Job(AbstractJob):  # in job
    def __init__(self, job_type, timing, handle):
